...
static int get_lcd_name(void)
{
	...
	if (!rc) {
		...
		if (lcd_name_p) {
			...
			cts_info("%s:lcd name: %s\n", __func__, lcd_name_for_tp);
		}
	}
        ...
        unsigned long action, void *data)
{
    ...
    if (evdata && evdata->data) {
        ...
        } else {//else if (action == FB_EARLY_EVENT_BLANK) {
            blank = *(int *)evdata->data;
...
#endif
{
	...
	get_hardware_info_data(HWID_CTP_DRIVER, lcd_name_for_tp);
